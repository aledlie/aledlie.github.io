#!/usr/bin/env python3
"""
Build unified entity graph for PersonalSite (aledlie.com).
...
def main():
    # Configuration
    base_url = 'https://www.aledlie.com'
    schemas_dir = Path(__file__).parent

    # Find all JSON schema files (exclude the output files)
    json_files = [f for f in schemas_dir.glob('*.json')
                  if f.name not in ['unified-entity-graph.json', 'entity-graph-analysis.json']]
    json_files = sorted(json_files)

    print("=" * 80)
    print("Building PersonalSite (aledlie.com) Entity Graph")
    print("=" * 80)
    ...
    output_file = schemas_dir / 'unified-entity-graph.json'
    with open(output_file, 'w') as f:
        json.dump(graph, f, indent=2)
    print(f"\n✅ Unified entity graph saved to: {output_file.name}")

    # Save the analysis
    analysis_file = schemas_dir / 'entity-graph-analysis.json'
