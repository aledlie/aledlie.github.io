...
**The frozen number.** `smolagents` describes itself as roughly a thousand lines of core logic. That was true: at the v1.0.0 tag in December 2024, `agents.py` was 1,038 lines. Today it's 1,813, and the package around it went from 5,850 lines to 12,774. The README still says under a thousand. Minimalism is the library's entire pitch, so this is the load-bearing claim, and it has roughly doubled its way out of truth while every secondary write-up — Medium, DataCamp, the aggregators — repeats the figure without checking. The number was never wrong. It just stopped being now, and nothing in a README knows what now is.
